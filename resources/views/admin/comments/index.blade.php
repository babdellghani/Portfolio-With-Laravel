                                        ...
                                        <select name="blog_id" class="form-select">
                                            ...
                                            @foreach ($blogs as $blog)
                                                <option value="{{ $blog->id }}"
                                                    {{ request('blog_id') == $blog->id ? 'selected' : '' }}>
                    ...
                    <div class="card-body">
                                                        ...
                                                        <div class="flex-grow-1">
                                                            ...
                                                            @if ($comment->parent_id)
                                                                <small class="text-muted">
                                                                    Reply to:
                                                                    {{ Str::limit($comment->parent->content ?? 'Deleted comment', 50) }}
                                                                </small>
                                                            @endif
                                            ...
                                            <tr>
                                                <td colspan="7" class="text-center py-4">
                                                    <div class="text-muted">
                                                        ...
                                                        <p>Comments will appear here when users start engaging with your
                                                            blog posts.</p>
                                                    </div>
                                                </td>
                                            </tr>
                        ...
                        @foreach ($comments as $comment)
                            <form action="{{ route('admin.comments.approve', $comment) }}" method="POST"
                                id="approve-form-{{ $comment->id }}" style="display: none;">
                                @csrf
                                @method('PATCH')
                            </form>
                            <form action="{{ route('admin.comments.reject', $comment) }}" method="POST"
                                id="reject-form-{{ $comment->id }}" style="display: none;">
                                @csrf
                                @method('PATCH')
                            </form>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.getElementById('bulk-action-form').addEventListener('submit', function(e) {
                    ...
                    if (action === 'delete') {
                        if (!confirm(
                                'Are you sure you want to delete the selected comments? This will also delete all their replies.'
                            )) {
                            e.preventDefault();
                        }
                    }
