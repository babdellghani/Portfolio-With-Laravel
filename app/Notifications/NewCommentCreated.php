...
class NewCommentCreated extends Notification
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        return (new MailMessage)
            ->subject('New Comment Posted - ' . config('app.name'))
            ...
            ->line('Posted: ' . $this->comment->created_at->format('M d, Y h:i A'))
            ->action('View Comments', route('admin.comments.index'))
            ->line('Please review the new comment.');
    }
    ...
    public function toArray(object $notifiable): array
    {
        return [
            ...
            'message'      => $this->user->name . ' posted a new comment on "' . $this->comment->blog->title . '"',
