...
class BlogUpdated extends Notification
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        return (new MailMessage)
            ->subject('Blog Post Updated - ' . config('app.name'))
            ->greeting('Hello Admin!')
            ->line('A blog post has been updated.')
            ->line('Blog Details:')
            ->line('Title: ' . $this->blog->title)
            ->line('Author: ' . $this->user->name)
            ->line('Status: ' . ucfirst($this->blog->status))
            ->line('Updated: ' . $this->blog->updated_at->format('M d, Y h:i A'))
            ->action('View Blog Posts', route('admin.blogs.index'))
            ->line('Please review the updated blog post.');
    }
    ...
    public function toArray(object $notifiable): array
    {
        return [
            ...
            'message'    => $this->user->name . ' updated the blog post: "' . $this->blog->title . '"',
